...
helpString = bcolors.WARNING + \
  ...
  "/     - Search.\n" \
  "L     - Export library as lyricLibrary.json.\n" \
  "q     - Quit." + bcolors.ENDC
...
function displaySet( disFormat )
{
  ...
  for( var i = 0;i < acc.length;i++ )
  {
    ...
    var strName; // "1. Name" "11. Name". If DISPLAY_MIN Want to prune to "1" or "11".
...
var scrollToElem; // current element that should be at the top of the screen.

/* Expand a song name if it was compressed */
...
function accordionClick( elem )
{
  ...
  if( !wasOpen )
  {
    ...
    scrollToElem.scrollIntoView(); /* Sometimes scrollIntoView() is 'off'. Redo after a delay to fix this case. */
    setTimeout( function() { scrollToElem.scrollIntoView(); }, 500 ); // Hack for occasional misses while resizing
  }
}
...
class libSong( object ):
  def __init__( self, name, artist, bpm, lyrics ):
    self.name = name
    self.artist = artist if artist else ""
    #self.bpm = bpm if bpm else 0
    #self.lyrics = lyrics
    # self.id = ( self.name + "." + self.artist ).replace( " ", "_" )

# Export the library as lyricLibrary.json
def exportLibrary():
  global statusString
  ...
  for s in songLibrary:
    ...
    libSong[ "artist" ] = s.elements[ "Artist" ] if s.elements[ "Artist" ] else ""
    lyrics = ""
